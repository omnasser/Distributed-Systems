...
class TestHW3(unittest.TestCase):
    ...
    def test_a_get_shard_ids(self):
        ...
        print("heree")
        # self.assertEqual(response.status_code, 200, 'bad code')
        stat = 333

        try :
            self.assertEqual(stat, 200)
            print("test a1 was good") 
        except:
            print("test a1 was bad")



        self.assertEqual(stat, 200, 'bad code')
        shardIdsFromNode1 = responseInJson['shard-ids']
        self.assertEqual(len(shardIdsFromNode1), shardCount, 'nono88')

        # get the shard IDs from node5
        response = requests.get( 'http://localhost:8087/key-value-store-shard/shard-ids')
        responseInJson = response.json()
        self.assertEqual(response.status_code, 200, '2bad code')
        shardIdsFromNode5 = responseInJson['shard-ids']
        self.assertTrue(compareLists(shardIdsFromNode5, shardIdsFromNode1), 'nono55')

        # get the shard IDs from node6
        response = requests.get( 'http://localhost:8088/key-value-store-shard/shard-ids')
        responseInJson = response.json()
        self.assertEqual(response.status_code, 200, 'bad code3')
        shardIdsFromNode6 = responseInJson['shard-ids']
        self.assertTrue(compareLists(shardIdsFromNode6, shardIdsFromNode1), 'nono')
