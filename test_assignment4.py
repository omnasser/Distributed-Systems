...
class TestHW3(unittest.TestCase):
    ...
    def test_a_get_shard_ids(self):
        ...
        try:
            ...
            print("error in get of ('http://localhost:8082/key-value-store-shard/shard-ids')")

        
        self.assertEqual(response.status_code, 200, 'testA: wrong code 1 ')
        shardIdsFromNode1 = responseInJson['shard-ids']
        self.assertEqual(len(shardIdsFromNode1), shardCount, 'testA: wrong len 1')
        ...
        try:
            ...
            print("get error: http://localhost:8087/key-value-store-shard/shard-ids'")

        self.assertEqual(response.status_code, 200, 'testA: wrong code 2 ')
        shardIdsFromNode5 = responseInJson['shard-ids']
        self.assertTrue(compareLists(shardIdsFromNode5, shardIdsFromNode1), 'testA: list no match 1')
        ...
        self.assertEqual(response.status_code, 200, 'testA: wrong code 3 ')
        shardIdsFromNode6 = responseInJson['shard-ids']
        self.assertTrue(compareLists(shardIdsFromNode6, shardIdsFromNode1), 'testA: list no match 2')
    ...
    def test_b_shard_id_members(self):
        ...
        try:
            ...
            print("get error: 'http://localhost:8083/key-value-store-shard/shard-id-members/' + shard1")

        self.assertEqual(response.status_code, 200, 'bad code b1')
        shard1Members = responseInJson['shard-id-members']
        print(len(shard1Members))
        self.assertGreater(len(shard1Members), 1,'bad len 1')
        ...
        try:
            ...
            print("'http://localhost:8084/key-value-store-shard/shard-id-members/' + shard2")
        
        self.assertEqual(response.status_code, 200, 'bad code b2')
        shard2Members = responseInJson['shard-id-members']
        print(len(shard1Members))
        self.assertGreater(len(shard2Members), 1, 'bad len 2')


        print("total:herer: ")
        print(len(nodeSocketAddressList))
        self.assertEqual(len(nodeSocketAddressList), len(shard1Members + shard2Members), 'bad len 3')
    ...
    def test_c_node_shard_id(self):
        ...
        try:
            ...
            print("get error: http://localhost:8082/key-value-store-shard/node-shard-id")

        self.assertEqual(response.status_code, 200, 'wrong code 1')

        node1ShardId = responseInJson['shard-id']

        self.assertTrue(node1ShardId in self.shardIdList, 'not in the list 1')

        if node1ShardId == shard1:
            self.assertTrue(nodeSocketAddressList[0] in self.shardsMemberList[0], 'node is noe lokking for 1')
        else:
            self.assertTrue(nodeSocketAddressList[0] in self.shardsMemberList[1], 'node is node looking for 2')
        ...
        try:
            ...
            print("get error: http://localhost:8083/key-value-store-shard/node-shard-id")


        self.assertEqual(response.status_code, 200, 'bad code 2')

        node2ShardId = responseInJson['shard-id']

        self.assertTrue(node2ShardId in self.shardIdList, 'not in list 2')

        if node2ShardId == shard1:
            self.assertTrue(nodeSocketAddressList[1] in self.shardsMemberList[0], 'node no match 3')
        else:
            self.assertTrue(nodeSocketAddressList[1] in self.shardsMemberList[1], 'node no match  4')
        ...
        try:
            ...
            print("http://localhost:8088/key-value-store-shard/node-shard-id'")

        
        self.assertEqual(response.status_code, 200, 'bad code 3')

        node6ShardId = responseInJson['shard-id']

        self.assertTrue(node6ShardId in self.shardIdList, 'not in list 3')

        if node6ShardId == shard1:
            self.assertTrue(nodeSocketAddressList[5] in self.shardsMemberList[0], 'no match 5')
        else:
            self.assertTrue(nodeSocketAddressList[5] in self.shardsMemberList[1], 'no match 6')
    ...
    def test_d_put_key_value_operation(self):
        ...
        for counter in range(self.keyCount):
            ...
            try:
                response = requests.put('http://localhost:' + nodeHostPortList[nodeIndex] + '/key-value-store/key' + str(counter), json={'value': "value" + str(counter), "causal-metadata": nextCausalMetadata}, timeout=60)
                try:
                    responseInJson = response.json()
                except:
            ...
            self.assertTrue(keyShardId in self.shardIdList, 'shard id not in list')
